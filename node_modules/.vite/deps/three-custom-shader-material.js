...
// node_modules/.pnpm/three-custom-shader-material@6.3.7_@react-three+fiber@9.1.2_@types+react@19.1.4_react-d_da17803661b2849d6dc9897cd998b016/node_modules/three-custom-shader-material/three-custom-shader-material.es.js
var import_jsx_runtime = __toESM(require_jsx_runtime());
var r = __toESM(require_react());

// node_modules/.pnpm/three-custom-shader-material@6.3.7_@react-three+fiber@9.1.2_@types+react@19.1.4_react-d_da17803661b2849d6dc9897cd998b016/node_modules/three-custom-shader-material/vanilla/three-custom-shader-material.es.js
var D = (
  /* glsl */
  `
...
var B = class extends Material {
  constructor({
    ...
    for (const t in f) {
      const l2 = f[t];
      (l2.get || l2.set) && Object.defineProperty(this, t, l2);
    }
  ...
  update({
    ...
    const R = s.__csm.prevOnBeforeCompile, o = (a, n, f) => {
      let t, l2 = "";
      if (n) {
        const r2 = n.search(/void\s+main\s*\(\s*\)\s*{/);
        if (r2 !== -1) {
          l2 = n.slice(0, r2);
          let c2 = 0, g = -1;
          for (let S = r2; S < n.length; S++)
            if (n[S] === "{" && c2++, n[S] === "}" && (c2--, c2 === 0)) {
              g = S;
              break;
            }
          if (g !== -1) {
            const S = n.slice(r2, g + 1);
...
` + t), a.includes("//~CSM_DEFAULTS")) {
        ...
        const r2 = a.lastIndexOf("//~CSM_MAIN_END");
        if (r2 !== -1) {
          const c2 = `
            ${t ? `${t}` : ""}
            //~CSM_MAIN_END
          `;
          a = a.slice(0, r2) + c2 + a.slice(r2);
        }
      } else {
        const r2 = /void\s*main\s*\(\s*\)\s*{/gm;
        a = a.replace(
          r2,
          `
          // THREE-CustomShaderMaterial by Faraz Shaikh: https://github.com/FarazzShaikh/THREE-CustomShaderMaterial
    ...
    s.onBeforeCompile = (a, n) => {
      R == null || R(a, n);
      const f = I || {}, t = s.type, l2 = t ? `#define IS_${t.toUpperCase()};
` : `#define IS_UNKNOWN;
`;
      a.vertexShader = l2 + `#define IS_VERTEX
` + a.vertexShader, a.fragmentShader = l2 + `#define IS_FRAGMENT
` + a.fragmentShader;
      const T = (r2) => {
        for (const c2 in r2) {
          const g = c2 === "*" || M && M.includes(c2);
          if (c2 === "*" || d && d.includes(c2) || g) {
            const p2 = F[c2];
            if (p2 && p2 !== "*" && (Array.isArray(p2) ? !p2.includes(t) : p2 !== t)) {
              console.error(
                `CustomShaderMaterial: ${c2} is not available in ${t}. Shader cannot compile.`
              );
              return;
            }
            const $ = r2[c2];
...
function p({
  ...
  }, [e2, t, i, s, f]), r.useEffect(() => () => o.dispose(), [o]), (0, import_jsx_runtime.jsx)(
    "primitive",
    {
      ref: m,
      attach: d ?? "material",
